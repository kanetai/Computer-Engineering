...
反復型開発プロセスの工程 | 使用するダイアグラム
:--- |:---
要求分析(requirements analysis) | ユースケース図、アクティビティ図
分析(analysis) | クラス図、シーケンス図、コミュニケーション図、ステートマシン図、オブジェクト図、相互作用概要図
...
ユーザーの**システムに対する機能要求**を聞いて、それを定義する。この段階では、システムの内部は**ブラックボックス化**して、システムの外から見た機能のみに着目する。**非機能要求も**洗い出す。
...
対象システムで実現予定の機能がすべてユースケースで表現されている必要がある。
...
- パッケージ間の双方向の依存関係を避ける（双方向依存のクラスをパッケージ内に集める）
...
クラス間の**静的**な関係、**論理的な集まり**を表現する。DBの設計ではER図の代替としても利用できる。クラス間の関係では、依頼・参照・使用・集約・**状態**など**より構造化された複雑な情報**を表現することができる。
...
- **動的分類(Dynamic Classification)：　**あるクラスのインスタンスがライフサイクルの途中で別のクラスに分類(鞍替え)されるという概念。`<<dynamic>>`キーワードを定義して、汎化セット名の近くにつけておくと良い。フラグやStateパターンなどで実装できる。 *例：　学生クラスのインスタンスが社会人クラスのインスタンスに変わる*
...
**相手側クラスのあるオブジェクト**が関連しているオブジェクト数(総インスタンス数ではない)。オブジェクト図(に相当する集合図)とER図のcrow's footを見ればイメージしやすい。
...
`{xor}` | 関連を点線で結んで、xor制約をつけることにより、どちらか一方のみが有効になることを示す。

*例* **多重分類((Multiple Classification[Specification]))** *: 汎化セットが複数あるので{overlapping}を付けて明示する*
...
- *{ordered}*　インスタンスがあるルールで並んでいるという関連端(association end)のプロパティ。**制約ではなく**メタ属性。
...
基本となる型をカテゴライズする型。**そのインスタンスが別のクラスのサブクラスでもあるもの**。ベースとなる型のインスタンス生成を制約するのが主な目的。べき型からなる構造を**知識レベル(knowledge level)**, ベースとなる型からなる構造を**操作レベル(operation level)**と呼ぶ。

*例:　*幾つかの個客種、商品種、注文種の組み合わせが決まっていて、それに反する注文は出来ない(インスタンスはない)。例えば、`customer type`のインスタンスは`customer`のサブクラス(`general customer`, `best customer` etc.)。
...
- [[改訂版] UMLモデリング技能認定試験<入門レベル(L1)>問題集 -UML2.0対応](http://www.amazon.co.jp/%E6%94%B9%E8%A8%82%E7%89%88-UML%E3%83%A2%E3%83%87%E3%83%AA%E3%83%B3%E3%82%B0%E6%8A%80%E8%83%BD%E8%AA%8D%E5%AE%9A%E8%A9%A6%E9%A8%93-%E5%85%A5%E9%96%80%E3%83%AC%E3%83%99%E3%83%AB-%E5%95%8F%E9%A1%8C%E9%9B%86--UML2-0%E5%AF%BE%E5%BF%9C/dp/4774132454/ref=sr_1_1?ie=UTF8&qid=1443222547&sr=8-1&keywords=uml%E3%83%A2%E3%83%87%E3%83%AA%E3%83%B3%E3%82%B0%E6%8A%80%E8%83%BD%E8%AA%8D%E5%AE%9A%E8%A9%A6%E9%A8%93)
	- 1.8, 2.15, 8.10, 14.9, 14.10, 14.15, 14.20, 15.7, 15.10
	- (3.22, 3.31, 3.33, 4.4, 4.14, 5.15, 6.7, 8.4, 8.9, 8.11, 10.12, 13.1, 13.4, 13.6, 13.8, 14.3, 14.4, 14.5, 14.17, 14.21, 15.6, 15.8, 15.11, 15.12, 16.6, 17.6, 17.11)
- [徹底攻略UMLモデリング技能認定試験問題集―L1(T1/T2)対応 (ITプロ/ITエンジニアのための徹底攻略)](http://www.amazon.co.jp/gp/product/4844324829?psc=1&redirect=true&ref_=oh_aui_detailpage_o02_s00)
	- 13.23, 13.24
	- (2.9, 3.2, 3.7, 3.13, 3.20, 3.28, 4.2, 4.10, 5.7, 10.6, 11.1, 11.6, 12.2, 12.3, 12.7, 12.9, 13.1, 13.2, 13.6, 13.10, 13.13, 13.14, 14.8, 15.2, 18.1, 18.19, 20.4, 20.5, 20.9, 21.30, 21.34, 21.37, 22.3, 22.25, 22.30)

- [徹底攻略UMLモデリング技能認定試験問題集 L2対応](https://www.amazon.co.jp/%E5%BE%B9%E5%BA%95%E6%94%BB%E7%95%A5UML%E3%83%A2%E3%83%87%E3%83%AA%E3%83%B3%E3%82%B0%E6%8A%80%E8%83%BD%E8%AA%8D%E5%AE%9A%E8%A9%A6%E9%A8%93%E5%95%8F%E9%A1%8C%E9%9B%86-L2%E5%AF%BE%E5%BF%9C-%E6%A0%AA%E5%BC%8F%E4%BC%9A%E7%A4%BE%E3%82%AA%E3%83%BC%E3%82%B8%E3%82%B9%E7%B7%8F%E7%A0%94-%E7%AB%B9%E6%94%BF-%E6%98%AD%E5%88%A9/dp/4844335596/ref=sr_1_2?ie=UTF8&qid=1486283450&sr=8-2&keywords=umtp)
	- 1.5, 3.11, 4.2, 4.9, 4.10, 4.17, 6.4, 6.7, 6.10, 7.6, 8.1, 8.5, 8.6, 8.7, 8.8, 8.12, 8.15
	- (1.1, 1.7, 1.8, 1.10, 1.11, 2.2, 2.4, 2.5, 3.3, 3.9, 4.1, 4.3, 4.5, 4.11, 4.13, 4.14, 4.15, 4.16, 4.18, 5.5, 5.6, 5.9, 5.16, 6.1, 6.2, 6.8, 7.8, 7.11)
